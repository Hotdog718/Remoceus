...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if(!responses.includes(vote)) return message.channel.send("Enter a valid vote").then(m => m.delete({timeout: 5000}));
    ...
    if(!poll[vote]) return message.channel.send("Enter a valid vote").then(m => m.delete({timeout: 5000}));
