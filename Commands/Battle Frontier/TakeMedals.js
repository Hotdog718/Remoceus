...
module.exports = {
    ...
    if(message.deletable) message.delete();
		...
		if(!amount || isNaN(amount)) return message.channel.send("Please give an amount of medals to take").then(m => m.delete(5000));
        ...
        if(newMedals[type.toLowerCase()] < Math.abs(amount)){
          message.channel.send(`Cannot take ${amount} medals from ${mUser.user.tag} because they only have ${newMedals[type.toLowerCase()]} ${type.toLowerCase()} medals.`).then(m => m.delete(5000));
        }else{
          newMedals[type.toLowerCase()] -= Math.abs(amount);
          message.channel.send(`Took ${amount} ${type.toLowerCase()} medals from ${mUser.user.tag}`).then(m => m.delete(5000));
          newMedals.save().catch(err => console.log(err))
        }
			}else{
        if(medals[type.toLowerCase()] < Math.abs(amount)){
          message.channel.send(`Cannot take ${amount} medals from ${mUser.user.tag} because they only have ${medals[type.toLowerCase()]} ${type.toLowerCase()} medals.`).then(m => m.delete(5000));
        }else{
