...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    message.member.roles.add(role)
      .then(r => message.channel.send(`Added ${role.name} role to ${message.author.tag}`).then(m => m.delete({timeout: 5000}).catch(err => {})))
      .catch(err => message.channel.send(`Failed to add ${role.name} role to ${message.author.tag}`).then(m => m.delete({timeout: 5000}).catch(err => {})));
  }
}
